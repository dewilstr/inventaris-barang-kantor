<?php
// =============================================
// pageKasir.php – VERSI FIX TOTAL
// =============================================

// Aktifkan output buffering agar header() aman
ob_start();

// Tampilkan error selama debugging
...
<body>
            ...
            <div class="menu-sidebar2__content js-scrollbar1">
                <div class="account2">
                    <div class="image img-cir img-120">
                        <?php
                        // aman: cek apakah file foto ada, kalau nggak pakai default
                        $foto = 'img/' . ($auth['foto_user'] ?? 'default.png');
                        if (!file_exists($foto)) {
                            $foto = 'img/default.png';
                        }
                        ?>
                        <img src="<?= htmlspecialchars($foto) ?>" alt="user" />
                    </div>
                    <h4 class="name"><?= htmlspecialchars($auth['nama_user'] ?? 'Guest') ?></h4>
                </div>
 ...
 <div class="page-container2">
        ...
        <div class="section__content section__content--p30">
            ...
            <aside class="menu-sidebar2 js-right-sidebar d-block d-lg-none">
                ...
                <div class="menu-sidebar2__content js-scrollbar2">
                    <div class="account2">
                        <div class="image img-cir img-120">
                            <img src="<?= htmlspecialchars($foto) ?>" alt="John Doe" />
                        </div>
                        <h4 class="name"><?= htmlspecialchars($auth['nama_user'] ?? 'Guest') ?></h4>
    ...
    <script>
      ...
      // Handler logout yang tahan banting: support SweetAlert (Promise), SweetAlert v1 (callback), dan fallback confirm
      $(document).ready(function(){
        $('#forLogout').on('click', function(e){
          e.preventDefault();
          ...
          // 1) coba swal yang mengembalikan Promise (sweetalert modern)
          try {
            var maybePromise = swal(swalOptions);
            if (maybePromise && typeof maybePromise.then === 'function') {
              maybePromise.then(function(willLogout){
                if (willLogout) {
          ...
          // 2) coba sweetalert v1 (callback style)
          try {
            swal(swalOptions, function(willLogout){
              if (willLogout) {
                window.location.href = "?logout";
              }
            });
...
function loadNotif() {
    $.getJSON("config/notif.php", function(data) {
        ...
        // ⬅ TEMPAT NOTIF MASUK (WAJIB SESUAI)
