...
if (isset($_POST['getSave'])) {
    ...
    if (empty($kd_distributor) || empty($nama_distributor) || empty($nohp_distributor) || empty($alamat)) {
        $response = ['response'=>'negative','alert'=>'Lengkapi field'];
    } else {
        $validno = substr($nohp_distributor, 0, 2);
        if ($validno != "08") {
            $response = ['response'=>'negative','alert'=>'Masukkan no hp yang valid'];
        } elseif (strlen($nohp_distributor) < 11) {
            $response = ['response'=>'negative','alert'=>'Masukkan minimal 11 digit no hp'];
        } else {
            // ✅ NAMA KOLOM
            $columns = "kd_distributor, nama_distributor, alamat, no_telp";

            // ✅ ISI DATA
            $values  = "'$kd_distributor','$nama_distributor','$alamat','$nohp_distributor'";
